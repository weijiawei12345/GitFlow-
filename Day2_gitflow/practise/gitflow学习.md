...
冲突（Conflict） 是 Git 在合并代码时，发现两个不同的提交对同一文件的同一区域做了不同的修改，且无法自动判断应该保留哪一个版本时，暂停合并过程并请求人工介入的一种机制。
如果本地提交存在未推送的时候，此时如果远程对同一份文件有修改并提交推送，则本地提交在推送前，拉取最新的时候，发生冲突概率最大，如果本地处于推送完毕，且本地没有待推送的提交的文件，则会直接被远程最新的拉取覆盖。
冲突解决的方式：
保留自己的	删除远程内容，保留自己的	你的版本胜出
保留远程的	删除自己的内容，保留远程的	远程版本胜出
混合保留	手动组合两者内容	自定义版本
冲突文件内容：
<<<<<<< HEAD
你的内容
